...
#define HP_PID  0xf440

#define TOSHIBA_VID  0x0930
#define TOSHIBA_PID  0x6540
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	else if((id->idProduct == TOSHIBA_PID)&(id->idVendor == TOSHIBA_VID))
	{
		printk(KERN_INFO "Known TOSHIBA USB drive detected.\n");
	}
	else{
